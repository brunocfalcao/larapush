...
When you install Larapush on your web server, it installs Laravel Passport and generates a specific client token that will be unique. Henceforth before any transaction is made by your client, it need to first request a new client grant token and then use that token on the very next operation. Additionally there is also a token that is passed on each request that needs to be the same between your local computer and the web server. If not, the transaction aborts in error.
